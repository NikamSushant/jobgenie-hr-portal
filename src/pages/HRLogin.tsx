...
export default function HRLogin() {
  ...
  const [form, setForm] = useState({
    ...
    password: '', full_name: '', company_name: '',
    ...
    <div style={{ minHeight: '100vh', background: '#080810', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .cbx { width:18px; height:18px; border:1px solid rgba(200,169,110,0.3); border-radius:4px; background:rgba(255,255,255,0.03); display:flex; align-items:center; justify-content:center; transition:all 0.2s; flex-shrink:0; }
        .cbx.on { background:rgba(200,169,110,0.15); border-color:rgba(200,169,110,0.6); }
      `}</style>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} style={{ width: '100%', maxWidth: 480 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 48, fontWeight: 300, background: 'linear-gradient(135deg,#c8a96e,#e8c87e)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1 }}>JobGenie</div>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, letterSpacing: '0.35em', color: 'rgba(200,169,110,0.5)', textTransform: 'uppercase', marginTop: 6 }}>HR Portal</div>
          <div style={{ marginTop: 8, fontSize: 13, color: 'rgba(255,255,255,0.25)', fontFamily: 'Outfit, sans-serif' }}>Discover top talent from JobGenie AI</div>
        </div>
            ...
            {mode === 'forgot' ? (
              <motion.div key="forgot" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <button onClick={() => { setMode('login'); setError(''); setSuccess('') }}
                  style={{ background: 'none', border: 'none', color: 'rgba(200,169,110,0.6)', fontFamily: 'DM Mono, monospace', fontSize: 11, letterSpacing: '0.15em', cursor: 'pointer', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ← Back to Sign In
                </button>
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 26, fontWeight: 300, color: '#e8e2d9', marginBottom: 6 }}>Reset Password</div>
                  <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', fontFamily: 'Outfit, sans-serif' }}>Enter your email — we'll send a reset link.</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div>
                    <label className="lbl">Email</label>
                    <input className="inp" type="email" placeholder="hr@company.com" value={form.email} onChange={e => set('email', e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
                  </div>
                  {error && <div style={{ padding: '10px 14px', background: 'rgba(226,75,74,0.1)', border: '1px solid rgba(226,75,74,0.2)', borderRadius: 6, color: '#f09595', fontSize: 13 }}>{error}</div>}
                  {success && <div style={{ padding: '10px 14px', background: 'rgba(40,200,64,0.08)', border: '1px solid rgba(40,200,64,0.2)', borderRadius: 6, color: 'rgba(40,200,64,0.8)', fontSize: 13 }}>✓ {success}</div>}
                  {!success && <button className="btn-gold" onClick={handleSubmit} disabled={loading}>{loading ? 'Sending...' : 'Send Reset Link →'}</button>}
                </div>
              </motion.div>
              ...
              <motion.div key="auth" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <input className="inp" type="password" placeholder="••••••••" value={form.password} onChange={e => set('password', e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
                  </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => setRememberMe(r => !r)}>
                        <div className={`cbx ${rememberMe ? 'on' : ''}`}>
                          {rememberMe && <span style={{ color: '#c8a96e', fontSize: 11 }}>✓</span>}
                        </div>
                        <span style={{ fontFamily: 'Outfit, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>Remember me</span>
                      </div>
                      <button onClick={() => { setMode('forgot'); setError(''); setSuccess('') }}
                        style={{ background: 'none', border: 'none', color: 'rgba(200,169,110,0.6)', fontFamily: 'Outfit, sans-serif', fontSize: 13, cursor: 'pointer', textDecoration: 'underline' }}>
                        Forgot password?
                      </button>
                    </div>
                  )}
                ...
                <div style={{ textAlign: 'center', marginTop: 20 }}>
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', fontFamily: 'Outfit, sans-serif' }}>{mode === 'login' ? 'New here? ' : 'Already registered? '}</span>
