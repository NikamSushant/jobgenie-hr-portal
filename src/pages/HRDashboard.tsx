  ...
  const fetchCandidates = useCallback(async (skill: string, loc: string, exp: string) => {
    ...
    try {
      ...
      if (skill) {
        ...
        results = results.filter((c: any) =>
          (c.skills || []).some((s: string) => s.toLowerCase().includes(sl))
        )
      }
    ...
    <div style={{ minHeight: '100vh', background: '#0f0f0f', color: '#fff', fontFamily: "'Outfit', 'Inter', sans-serif" }}>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        .hr-inp { width:100%; padding:11px 16px; background:#1e1e1e; border:1.5px solid #2a2a2a; border-radius:12px; color:#fff; font-family:'Outfit',sans-serif; font-size:14px; outline:none; transition:border-color 0.2s; }
        .hr-inp:focus { border-color:#f59e0b; }
        .hr-inp::placeholder { color:#444; }
        .hr-sel { width:100%; padding:11px 16px; background:#1e1e1e; border:1.5px solid #2a2a2a; border-radius:12px; color:#fff; font-family:'Outfit',sans-serif; font-size:14px; outline:none; cursor:pointer; appearance:none; }
        .hr-sel option { background:#1a1a1a; }
        .candidate-card { background:#1a1a1a; border:1.5px solid #252525; border-radius:20px; padding:22px; cursor:pointer; transition:border-color 0.25s, transform 0.2s, box-shadow 0.2s; }
        .candidate-card:hover { border-color:#f59e0b; transform:translateY(-3px); box-shadow:0 12px 32px rgba(245,158,11,0.12); }
        .skill-tag { padding:5px 12px; border-radius:8px; font-size:11px; font-weight:600; letter-spacing:0.03em; }
        .btn-orange { padding:11px 24px; background:linear-gradient(135deg,#f59e0b,#f97316); border:none; border-radius:12px; color:#0f0f0f; font-family:'Outfit',sans-serif; font-size:13px; font-weight:700; cursor:pointer; transition:all 0.25s; }
        ...
        .btn-outline { padding:11px 24px; border:1.5px solid #2a2a2a; background:transparent; border-radius:12px; color:#888; font-family:'Outfit',sans-serif; font-size:13px; font-weight:600; cursor:pointer; transition:all 0.25s; }
      ...
      {/* NAV */}
      <nav style={{ position:'sticky', top:0, zIndex:100, background:'rgba(15,15,15,0.95)', backdropFilter:'blur(20px)', borderBottom:'1.5px solid #1e1e1e', padding:'14px 48px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:16 }}>
          {hrProfile && (
            <div style={{ background:'#1a1a1a', border:'1.5px solid #252525', borderRadius:12, padding:'8px 16px', textAlign:'right' }}>
              <div style={{ fontSize:13, fontWeight:600, color:'#fff' }}>{hrProfile.full_name}</div>
              <div style={{ fontSize:11, color:'#f59e0b', fontWeight:600 }}>{hrProfile.company_name}</div>
            </div>
          )}
          <button onClick={handleSignOut} style={{ background:'#1a1a1a', border:'1.5px solid #252525', borderRadius:12, padding:'10px 18px', color:'#555', fontSize:13, fontWeight:600, cursor:'pointer' }}>
            Sign Out
          </button>
        </div>
      </nav>
      ...
      <div style={{ maxWidth:1240, margin:'0 auto', padding:'48px 48px 80px' }}>
        ...
        <div style={{ marginBottom:40 }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(245,158,11,0.1)', border:'1.5px solid rgba(245,158,11,0.2)', borderRadius:30, padding:'6px 14px', marginBottom:16 }}>
        ...
        {/* SEARCH */}
        <div style={{ background:'#1a1a1a', border:'1.5px solid #252525', borderRadius:20, padding:'24px 28px', marginBottom:32 }}>
        ...
        {/* TABS */}
        <div style={{ display:'flex', gap:4, marginBottom:28, background:'#1a1a1a', borderRadius:14, padding:4, width:'fit-content', border:'1.5px solid #252525' }}>
          {(['search','saved'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={{ padding:'10px 24px', border:'none', borderRadius:11, cursor:'pointer', fontFamily:'Outfit,sans-serif', fontSize:13, fontWeight:700, transition:'all 0.25s',
              ...
              {tab === 'search' ? 'All Candidates' : 'Saved'}
              {tab==='saved' && saved.size>0 && (
                <span style={{ marginLeft:8, background:'rgba(0,0,0,0.2)', color:'#0f0f0f', borderRadius:20, padding:'2px 8px', fontSize:11, fontWeight:800 }}>{saved.size}</span>
              )}
            </button>
          ))}
        </div>
            ...
            {displayList.map((c, i) => (
              <motion.div key={c.user_id} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*0.04 }}
                ...
                <div style={{ display:'flex', gap:12, marginBottom:12, flexWrap:'wrap' }}>
                  {c.location && <span style={{ fontSize:11, color:'#555', fontWeight:600 }}>📍 {c.location}</span>}
                  {c.experience_years && <span style={{ fontSize:11, color:'#555', fontWeight:600 }}>⏱ {c.experience_years}+ yrs</span>}
                </div>
                {c.bio && (
                  <p style={{ fontSize:12, color:'#555', lineHeight:1.6, marginBottom:14, overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as any }}>{c.bio}</p>
                )}
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:14 }}>
                  {(c.skills||[]).slice(0,4).map((s,si) => (
                    <span key={s} className="skill-tag" style={{ background:`${SKILL_COLORS[si%SKILL_COLORS.length]}15`, border:`1.5px solid ${SKILL_COLORS[si%SKILL_COLORS.length]}30`, color:SKILL_COLORS[si%SKILL_COLORS.length] }}>{s}</span>
                  ))}
                  {(c.skills||[]).length>4 && <span className="skill-tag" style={{ background:'#222', border:'1.5px solid #2a2a2a', color:'#555' }}>+{(c.skills||[]).length-4}</span>}
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:12, borderTop:'1px solid #222' }}>
                  <span style={{ fontSize:10, fontWeight:600, letterSpacing:'0.08em', color:'#333', textTransform:'uppercase' }}>via JobGenie AI</span>
        ...
        {selected && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            <motion.div initial={{ scale:0.94, y:24 }} animate={{ scale:1, y:0 }} exit={{ scale:0.94 }}
              style={{ background:'#1a1a1a', border:'1.5px solid #252525', borderRadius:24, padding:36, maxWidth:580, width:'100%', maxHeight:'88vh', overflowY:'auto', position:'relative' }}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:24 }}>
                {[['📍 Location', selected.location], ['⏱ Experience', selected.experience_years ? `${selected.experience_years}+ years` : null]].filter(([,v])=>v).map(([k,v])=>(
                  <div key={k as string} style={{ background:'#222', border:'1.5px solid #2a2a2a', borderRadius:14, padding:'14px 18px' }}>
                ...
                <div style={{ marginBottom:28 }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                    {(selected.skills||[]).map((s,si)=>(
                      <span key={s} className="skill-tag" style={{ background:`${SKILL_COLORS[si%SKILL_COLORS.length]}15`, border:`1.5px solid ${SKILL_COLORS[si%SKILL_COLORS.length]}30`, color:SKILL_COLORS[si%SKILL_COLORS.length], fontSize:12, padding:'6px 14px' }}>{s}</span>
                    ))}
                  </div>
                </div>
              )}
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                ...
                <button className="btn-orange" style={{ marginLeft:'auto' }} onClick={() => sendContact(selected)} disabled={contactSent.has(selected.user_id)}>
                  {contactSent.has(selected.user_id) ? '✓ Contacted' : 'Contact →'}
                </button>
              </div>
              {contactSent.has(selected.user_id) && (
                <div style={{ marginTop:14, padding:'12px 16px', background:'rgba(52,211,153,0.08)', border:'1.5px solid rgba(52,211,153,0.2)', borderRadius:12, fontSize:13, color:'#34d399', fontWeight:600 }}>
